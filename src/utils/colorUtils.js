 ...
 * Maps code metrics to architectural building types for visual variety
 * 
 * Types:
 * 0 - Megastructure: LOC > 5000 or complexity > 100
 * 1 - Skyscraper: LOC 1000-5000 or complexity 50-100
 * 2 - Office Tower: LOC 500-1000 or complexity 20-50
 * 3 - Commercial: LOC 200-500 or complexity 10-20
 * 4 - Residential: LOC 50-200 or complexity 5-10
 * 5 - Utility: LOC < 50 or complexity < 5
 */
export function getBuildingType(data) {
    const loc = data.metrics?.loc || data.dimensions?.height * 10 || 50
    const complexity = data.metrics?.complexity || 0
    
    // Primary classification by LOC, secondary by complexity
    if (loc > 5000 || complexity > 100) return 0  // Megastructure
    if (loc > 1000 || complexity > 50) return 1   // Skyscraper
    if (loc > 500 || complexity > 20) return 2    // Office Tower
    if (loc > 200 || complexity > 10) return 3    // Commercial
    if (loc > 50 || complexity > 5) return 4      // Residential
    return 5                                       // Utility
}

/**
 * Get district type from directory path
 * Used for zone-based ground coloring and district effects
 */
export function getDistrictType(path) {
    if (!path) return 'other'
    const p = path.toLowerCase()
    
    if (p.includes('core') || p.includes('engine') || p.includes('kernel')) return 'core'
    if (p.includes('api') || p.includes('service') || p.includes('server')) return 'commercial'
    if (p.includes('util') || p.includes('lib') || p.includes('pkg') || p.includes('helper')) return 'industrial'
    if (p.includes('component') || p.includes('ui') || p.includes('page') || p.includes('view')) return 'residential'
    if (p.includes('test') || p.includes('spec') || p.includes('doc') || p.includes('example')) return 'green'
