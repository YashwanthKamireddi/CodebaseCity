...
void main() {
    ...
    // === 1. ASPHALT BASE - Dark grey, not black ===
    vec3 asphalt = vec3(0.08, 0.08, 0.10);
    // Subtle noise texture
    float noise = fract(sin(dot(vec2(u * 0.5, v * 10.0), vec2(12.9898, 78.233))) * 43758.5453);
    asphalt += vec3(0.02) * noise;

    // === 2. LANE MARKINGS - White dashed lines ===
    // Center line
    float centerWidth = 0.015;
    float centerMask = smoothstep(0.5 - centerWidth, 0.5 - centerWidth * 0.5, v) 
                     * smoothstep(0.5 + centerWidth, 0.5 + centerWidth * 0.5, v);
    float dashLength = 6.0;
    float gapLength = 4.0;
    float dashPattern = step(dashLength, mod(u, dashLength + gapLength));
    float centerLine = (1.0 - centerMask) * (1.0 - dashPattern);
    
    // Edge lines (solid white)
    float edgeWidth = 0.02;
    float leftEdge = smoothstep(edgeWidth + 0.01, edgeWidth, v);
    float rightEdge = smoothstep(1.0 - edgeWidth - 0.01, 1.0 - edgeWidth, v);
    float edgeLines = leftEdge + rightEdge;

    // === 3. NEON GLOW STRIPS - Cyan accent at very edge ===
    float glowWidth = 0.008;
    float glowL = exp(-v * v * 800.0) * 0.8;
    float glowR = exp(-(1.0 - v) * (1.0 - v) * 800.0) * 0.8;
    float pulse = 0.7 + 0.3 * sin(u * 0.1 - uTime * 2.0);
    vec3 neonGlow = vec3(0.0, 0.9, 1.0) * (glowL + glowR) * pulse;

    // === 4. SUBTLE LANE GUIDES (for avenues) ===
    float laneGuide = 0.0;
    if (vIsAvenue > 0.5) {
        float lane1 = smoothstep(0.008, 0.0, abs(v - 0.33)) * 0.3;
        float lane2 = smoothstep(0.008, 0.0, abs(v - 0.67)) * 0.3;
        laneGuide = (lane1 + lane2) * step(mod(u, 3.0), 1.5);
    }

    // === 5. DIRECTIONAL ARROWS (subtle) ===
    float arrowSpacing = 40.0;
    float arrowU = mod(u - uTime * 5.0, arrowSpacing);
    float arrowMask = step(0.0, arrowU) * step(arrowU, 3.0);
    float arrowV = abs(v - 0.5);
    float arrowShape = step(arrowV, 0.08 - arrowU * 0.02) * arrowMask * 0.15;

    // === COMBINE ===
    vec3 color = asphalt;
    // White markings
    color = mix(color, vec3(0.9), centerLine * 0.8);
    color = mix(color, vec3(0.85), edgeLines * 0.7);
    color = mix(color, vec3(0.6), laneGuide);
    color = mix(color, vec3(0.5), arrowShape);
    // Neon accent
    color += neonGlow * 2.0;
